...
<a href="{{ $href }}"
    class="admin-nav-item flex items-center gap-3 px-3 py-2.5 text-[13px] font-semibold rounded-xl transition-all duration-200 {{ $active ? 'active text-white bg-white/[.08]' : 'text-white/45 hover:text-white/80 hover:bg-white/[.04]' }}">
    <div class="flex items-center justify-center w-6 h-6 shrink-0 {{ $active ? 'text-accent' : 'text-white/60' }}">
        ...
        @elseif($icon === 'message-square-quote')
            <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    <span x-show="$store.ui.sidebarOpen" class="font-bold tracking-tight whitespace-nowrap" x-transition:enter="transition ease-out duration-200"
