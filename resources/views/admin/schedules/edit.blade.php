<x-admin-form 
    ...
    backRoute="{{ route('admin.schedules.index') }}"
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="mass_type" required 
                    class="w-full bg-background border rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all font-bold text-primary">
                    <option value="sunday" {{ old('mass_type', $schedule->mass_type) == 'sunday' ? 'selected' : '' }}>Sunday</option>
                    <option value="weekday" {{ old('mass_type', $schedule->mass_type) == 'weekday' ? 'selected' : '' }}>Weekday</option>
                    <option value="saturday" {{ old('mass_type', $schedule->mass_type) == 'saturday' ? 'selected' : '' }}>Saturday</option>
                    <option value="anticipated" {{ old('mass_type', $schedule->mass_type) == 'anticipated' ? 'selected' : '' }}>Anticipated</option>
                    <option value="special" {{ old('mass_type', $schedule->mass_type) == 'special' ? 'selected' : '' }}>Special</option>
                </select>
            </div>
            @php
                $existingTimes = old('time', is_array($schedule->time) ? $schedule->time : [$schedule->time]);
                // Ensure format H:i for the input
            ...
            @endphp
            <div x-data="{ times: {{ json_encode($formattedTimes ?: ['']) }} }">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 border rounded-md p-4 bg-background">
                @php $selectedDays = old('day_of_week', $schedule->day_of_week ?? []); @endphp
        ...
        <div class="flex items-center gap-3 py-2">
            ...
            <label for="is_active" class="text-sm font-bold text-primary cursor-pointer">Active and Visible on Website</label>
        </div>
    </div>
</x-admin-form>
